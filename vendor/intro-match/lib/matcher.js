import { MEL_BANDS, fftIterative, frameFeatures } from './audio-features.js';
import {
  extendIntroEndAtFadeCutHold,
  extendIntroEndAtSilentStaticHold,
  formatSecondRange,
  frameToSeconds,
  getPostEndStaticBoundaryContext as getPostEndStaticBoundaryContextBase,
  hasBoundaryAudioWake,
  hasFadingTailBeforeBoundary,
  parseScdetFrames,
  postProcessIntroFromSharedAudio as postProcessIntroFromSharedAudioBase,
  trimIntroEndAtPreJingleSilence,
  trimIntroStartAtBleedSceneChange as trimIntroStartAtBleedSceneChangeBase,
} from './intro-postprocess.js';
import {
  clamp,
  cosineSim,
  dotProduct,
...
const ANALYZE_SECONDS = 300;
const SAMPLE_RATE = 8000;
const FRAME_WINDOW_SECONDS = 0.5;
const FRAME_HOP_SECONDS = 0.25;
const MIN_INTRO_SECONDS = 20;
const MAX_INTRO_SECONDS = 150;
export const MAX_REFERENCE_FILES = 4;

// Match config
const DIAGONAL_SMOOTH_RADIUS = 1;
const RUN_GAP_FRAMES = 2;
const CONSENSUS_TOLERANCE_SECONDS = 4;
const DISTINCT_WINDOW_SECONDS = 8;
const MAX_RUNS_PER_REFERENCE = 12;

// Boundary config
const BOUNDARY_SEARCH_SECONDS = 6;
const BOUNDARY_WINDOW_SECONDS = 2;
const BOUNDARY_OUTWARD_SECONDS = 5.0;
const DIAGONAL_WALK_SECONDS = 12.0;
const DIAGONAL_WALK_SCORE_TOLERANCE = 0.04;
const DIAGONAL_WALK_DISTANCE_PENALTY = 0.006;
...
export class IntroMatchError extends Error {
...
// Turns raw mel frames into comparison-ready vectors by adding temporal deltas,
// normalizing each feature across the episode, then unit-normalizing each frame.
function buildNormalizedMatchFrames(melFrames) {
  ...
  for (let column = 0; column < featureCount; column += 1) {
    ...
    inverseStddevs[column] = 1 / Math.sqrt(variance > 1e-12 ? variance : 1.0);
  }
...
function finalizeDiagonalRun({
  ...
  const score =
    length * Math.max(0.02, meanSimilarity - baseline + 0.08) * (0.7 + consistency * 0.6);
...
function intervalClose(a, b, toleranceFrames) {
  return (
    Math.abs(a.mainStart - b.mainStart) <= toleranceFrames &&
    Math.abs(a.mainEnd - b.mainEnd) <= toleranceFrames
  );
}
...
function mergeRelatedRuns(runs, maxLen) {
  const deltaToleranceFrames = Math.round(1.25 / FRAME_HOP_SECONDS);
  const gapToleranceFrames = Math.round(2 / FRAME_HOP_SECONDS);
  const merged = [...runs].sort((a, b) =>
    lexicographicDescending(
  ...
  while (changed) {
    ...
    for (let i = 0; i < merged.length; i += 1) {
      for (let j = i + 1; j < merged.length; j += 1) {
        ...
        if (
          overlap < Math.max(0, Math.min(a.length, b.length) * 0.15) &&
          gap > gapToleranceFrames
        ) {
          continue;
        }
        ...
        const meanSimilarity =
          (a.meanSimilarity * a.length + b.meanSimilarity * b.length) /
          Math.max(1, a.length + b.length);
        const consistency =
          (a.consistency * a.length + b.consistency * b.length) / Math.max(1, a.length + b.length);
        ...
        merged[i] = {
          ...
          score:
            unionLength * Math.max(0.02, meanSimilarity - 0.28 + consistency * 0.05) +
            (a.score + b.score) * 0.15,
        };
        merged.splice(j, 1);
        changed = true;
        break;
...
function findPairwiseRuns(mainEpisode, refEpisode, minLen, maxLen, refIndex) {
  ...
  const toleranceFrames = Math.round(CONSENSUS_TOLERANCE_SECONDS / FRAME_HOP_SECONDS);
...
function buildCandidateFromRuns(runs, totalRefs, minLen, maxLen) {
  ...
  const consensusScore =
    totalRefs === 1 ? 0.75 : clamp(1 - (startSpread + endSpread) / Math.max(1, duration), 0.0, 1.0);

  const totalScore =
    duration * Math.max(0.02, similarityScore - 0.34 + consistency * 0.04) +
    overlap * 0.7 +
    consensusScore * 8 +
    referenceSupport * 5 -
    (startSpread + endSpread) * 0.3 -
    mainStart * 0.01;
  ...
  return {
    ...
    refs: runs.map((run) => ({
      ...
      startEdgeSimilarity: 0.0,
      endEdgeSimilarity: 0.0,
      pairScore: run.score,
...
function runsAgreeOnTimeline(a, b, minLen, toleranceFrames) {
  ...
  return (
    overlap >= Math.floor(minLen * 0.5) ||
    (startSpread <= toleranceFrames && endSpread <= toleranceFrames)
  );
}
...
function chooseBestConsensusRun(seed, pair, selectedRuns, minLen, toleranceFrames) {
  ...
  for (const run of pair.runs) {
    const agreedWithAll = selectedRuns.every((selected) =>
      runsAgreeOnTimeline(selected, run, minLen, toleranceFrames),
    );
...
function buildConsensusCandidates(pairwiseRuns, minLen, maxLen) {
  if (pairwiseRuns.length === 1) {
    return pairwiseRuns[0].runs
      .map((run) =>
        buildCandidateFromRuns([{ ...run, refIndex: pairwiseRuns[0].refIndex }], 1, minLen, maxLen),
      )
      .filter(Boolean);
  }
  ...
  const candidates = [];
  const toleranceFrames = Math.round(CONSENSUS_TOLERANCE_SECONDS / FRAME_HOP_SECONDS);
  const allRuns = pairwiseRuns.flatMap((pair) =>
    pair.runs.map((run) => ({
      ...run,
  ...
  for (const candidate of candidates) {
    if (
      deduped.some(
        (other) =>
          Math.abs(candidate.mainStart - other.mainStart) <= toleranceFrames &&
          Math.abs(candidate.mainEnd - other.mainEnd) <= toleranceFrames,
      )
    ) {
      continue;
    }
...
function boundaryObjective(pairwise, delta, start, end, mode) {
  const window = Math.max(2, Math.round(BOUNDARY_WINDOW_SECONDS / FRAME_HOP_SECONDS));
  const matrix = pairwise.similarity;

  if (mode === 'start') {
    const inside = diagonalAverage(matrix, delta, start, Math.min(end, start + window));
    const outside = diagonalAverage(matrix, delta, Math.max(0, start - window), start);
...
function enrichReferenceMatches(candidate, pairwiseRuns) {
  const refs = candidate.refs.map((detail) => {
    ...
    return {
      ...
      startEdgeSimilarity: startMetrics?.edge ?? 0.0,
      endEdgeSimilarity: endMetrics?.edge ?? 0.0,
    };
  });
...
function findWalkedForwardEnd(candidate, pairwiseRuns, baseEnd, maxAllowedEnd) {
  ...
  const walkLimit = Math.min(
    maxAllowedEnd,
    baseEnd + Math.round(DIAGONAL_WALK_SECONDS / FRAME_HOP_SECONDS),
  );
...
function meanMelFrameDelta(mainEpisode, start, end) {
  ...
  if (mainEpisode.boundaryDeltaPrefix) {
    ...
    return clampedEnd > clampedStart
      ? rangeMean(mainEpisode.boundaryDeltaPrefix, clampedStart, clampedEnd)
      : 0.0;
  }
...
function getPostEndStaticBoundaryContext(args) {
  return getPostEndStaticBoundaryContextBase({
    ...args,
    getMeanDiagonalSimilarityForRefs: meanDiagonalSimilarityForRefs,
  });
}
...
async function trimIntroStartAtBleedSceneChange({
  ...
  return trimIntroStartAtBleedSceneChangeBase({
    intro,
    sharedAudio,
    mainEpisode,
    pairwiseRuns,
...
async function postProcessIntroFromSharedAudio(
  ...
  return postProcessIntroFromSharedAudioBase({
    sharedAudio,
    mainEpisode,
    pairwiseRuns,
    minLen,
...
function findBestBoundaryFrame({
  ...
  const originalFrame =
    mode === 'start' ? originalCandidate.mainStart : originalCandidate.mainEnd;
...
function refineSharedAudioBoundaries(candidate, pairwiseRuns, minLen, maxLen, mainFrames) {
  const inwardFrames = Math.round(BOUNDARY_SEARCH_SECONDS / FRAME_HOP_SECONDS);
  const outwardFrames = Math.round(BOUNDARY_OUTWARD_SECONDS / FRAME_HOP_SECONDS);
...
function isDistinctCandidate(a, b, tolerance = DISTINCT_WINDOW_SECONDS) {
  const toleranceFrames = Math.max(1, Math.round(tolerance / FRAME_HOP_SECONDS));
  return (
    Math.abs(a.mainStart - b.mainStart) >= toleranceFrames ||
    Math.abs(a.mainEnd - b.mainEnd) >= toleranceFrames
  );
...
function computeConfidence(best, candidates) {
  ...
  for (let index = 1; index < candidates.length; index += 1) {
    ...
    if (isDistinctCandidate(best, candidate)) {
      altGap = Math.max(0.0, best.totalScore - candidate.totalScore);
      break;
    }
  }

  const simConf = clamp((best.similarityScore - 0.42) / 0.2, 0.0, 1.0);
  const supportConf = clamp((best.referenceSupport - 0.45) / 0.5, 0.0, 1.0);
  const overlapConf = clamp((best.overlapRatio - 0.3) / 0.45, 0.0, 1.0);
  const consensusConf = clamp(best.consensusScore, 0.0, 1.0);
  const boundaryConf = clamp((best.boundaryScore - 0.02) / 0.18, 0.0, 1.0);
  const gapConf = clamp((altGap - 1.5) / 8.0, 0.0, 1.0);
...
async function buildEpisodeFeatures(path, analyzeSeconds, sampleRate, runCommand) {
  ...
  const frameSize = Math.trunc(sampleRate * FRAME_WINDOW_SECONDS);
  const hopSize = Math.trunc(sampleRate * FRAME_HOP_SECONDS);
...
async function buildEpisodeSetFeatures(mainFile, refFiles, analyzeSeconds, sampleRate, runCommand) {
  const [mainEpisode, ...refEpisodes] = await Promise.all([
    ...
    ...refFiles.map((refFile) =>
      buildEpisodeFeatures(refFile, analyzeSeconds, sampleRate, runCommand),
    ),
  ]);
...
function deriveMatchFrameBounds(minIntroSec, maxIntroSec) {
  const minLen = Math.max(1, Math.floor(minIntroSec / FRAME_HOP_SECONDS));
  const maxLen = Math.max(minLen, Math.floor(maxIntroSec / FRAME_HOP_SECONDS));
...
function ensureEpisodeHasMatchableVariance(mainEpisode) {
  if (mainEpisode.volumeVariance < 1.0) {
    throw new IntroMatchError(
      'NO_VALID_CANDIDATES',
...
function compareEpisodeAgainstReferences(mainEpisode, refEpisodes, minLen, maxLen) {
  return refEpisodes.map((episode, index) =>
    findPairwiseRuns(mainEpisode, episode, minLen, maxLen, index + 1),
  );
}
...
function refineSharedAudioCandidates(rawCandidates, pairwiseRuns, minLen, maxLen, mainFrameCount) {
  return rawCandidates
    .slice(0, 8)
    .map((candidate) =>
      refineSharedAudioBoundaries(candidate, pairwiseRuns, minLen, maxLen, mainFrameCount),
    )
    .map((candidate) => ({
      ...candidate,
      totalScore:
        candidate.totalScore +
        candidate.boundaryScore * 6 +
        candidate.similarityScore * 2 +
        candidate.overlapRatio * 2,
    }));
}
...
function formatOutput(mainFile, refFiles, result, minConfidence) {
  ...
  const output = {
    ...
    reference_matches: result.sharedAudio.refs.map((detail) => ({
      ...
      ref_start_seconds: round4(detail.refStart * FRAME_HOP_SECONDS),
      ref_end_seconds: round4(detail.refEnd * FRAME_HOP_SECONDS),
      ref_start_hms: secondsToHms(detail.refStart * FRAME_HOP_SECONDS),
      ref_end_hms: secondsToHms(detail.refEnd * FRAME_HOP_SECONDS),
...
export async function findIntroMatch({ mainFile, refFiles, options = {}, runCommand }) {
  ...
  const intro = await postProcessIntroFromSharedAudio(
...
export const __testUtils = {
  FRAME_HOP_SECONDS,
  MEL_BANDS,
  buildEpisodeFeatures,
  buildEpisodeSetFeatures,
  getPostEndStaticBoundaryContext,
  buildConsensusCandidates,
  buildPairwiseSimilarity,
  buildSharedAudioCandidates,
  compareEpisodeAgainstReferences,
  cosineSim,
  deriveMatchFrameBounds,
  fftIterative,
  findPairwiseRuns,
  formatOutput,
  frameFeatures,
  mean,
  median,
  ensureEpisodeHasMatchableVariance,
  extendIntroEndAtFadeCutHold,
  extendIntroEndAtSilentStaticHold,
  hasBoundaryAudioWake,
  hasFadingTailBeforeBoundary,
  parseScdetFrames,
  postProcessIntroFromSharedAudio,
  trimIntroStartAtBleedSceneChange,
  trimIntroEndAtPreJingleSilence,
  robustNormalizeColumns,
  refineSharedAudioCandidates,
  runMatchingPipeline,
  safeLog10,
  scoreAndPickSharedAudioCandidate,
  secondsToHms,
  sharedAudioBoundaryScore,
  smoothSequence,
  refineSharedAudioBoundaries,
  toInt16Array,
};
